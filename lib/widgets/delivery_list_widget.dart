...
class DeliveryListView extends StatelessWidget {
  const DeliveryListView({super.key, required this.reachData, required this.reloadView});

  final List<DeliveryVo> reachData;
      ...
      child: Row(
        children: [
          ...
          InkWell(
            ...
            // btnOkOnPress: () {
            //   try {
            //     //DatabaseProvider.db.deleteDeliveryFromDB(widget.data.id ?? 0);
            //     helper.deleteDeliveryFromDB(widget.data.id ?? 0);
          ...
          InkWell(
            onTap: () {
              ...
              Navigator.of(context).push(MaterialPageRoute(builder: (builder)=>  EditDelivery(deliveryVo: widget.data)));
            },
            child: Container(
              height: 30,
              width: 30,
